...
if 2 < seg_tree.size:
    mem_usage_before = memory_usage()[0]
    update_time = timeit.timeit(lambda: seg_tree.update(1, 55, 2, 0, n - 1), number=1)
    mem_usage_after = memory_usage()[0]
    mem_diff = mem_usage_after - mem_usage_before
...
if 2 < seg_tree.size:
    mem_usage_before = memory_usage()[0]
    stabbing_query_time = timeit.timeit(lambda: seg_tree.stabbing_query(2), number=1)
    mem_usage_after = memory_usage()[0]
    mem_diff = mem_usage_after - mem_usage_before
    print(f"Memory used to perform a stabbing query: {max(0, mem_diff):.10f} MiB")
    print(f"Time taken to perform a stabbing query: {stabbing_query_time:.10f} seconds")
...
if ql < seg_tree.size and qr < seg_tree.size:
    ...
    print(f"Range query result for sum in range {query_range}: {range_query_result}")
